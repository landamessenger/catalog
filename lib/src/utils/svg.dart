...
class Svg extends ImageProvider<SvgImageKey> {
  ...
  Future<SvgImageKey> obtainKey(ImageConfiguration configuration) {
    final Color foregroundColor = this.foregroundColor ?? Colors.black;
    final Color backgroundColor = this.backgroundColor ?? Colors.white;
    final double scale = this.scale ?? configuration.devicePixelRatio ?? 1.0;
    final double logicWidth = size?.width ?? configuration.size?.width ?? 50;
    final double logicHeight = size?.height ?? configuration.size?.width ?? 50;
  ...
  static Future<String> _getSvgString(SvgImageKey key) async {
    ...
    switch (key.source) {
      ...
      case SvgSource.asset:
        var d = svgPaper;
        final hexColor =
            colorToHexStringNoAlpha(key.foregroundColor ?? Colors.white);
        d = d.replaceAll(
          'fill="#000"',
          'fill="$hexColor"',
        );
